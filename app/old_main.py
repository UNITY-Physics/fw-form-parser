def generate_qc_report_old (context, cover, api_key, input) :
    ...
    def acquisition_trends ():
            ...
            for session in subject.sessions():
                ...
                for metadata in metadata_fields.keys():
                    ...
                    if value is None or value == "" or value == "Not Found" or value == default.get(metadata):
                        row[metadata_fields[metadata]] = 0
                    else:
                        row[metadata_fields[metadata]] = 1
        ...
        asys_summary_df = df.groupby(["analysis", "state"]).size().unstack(fill_value=0)
        asys_summary_df["total"] = asys_summary_df.sum(axis=1)
        asys_summary_df = asys_summary_df.sort_values(by="total", ascending=True)

        # Normalize to get percentages (optional if you want percent bars)
        summary_percent = asys_summary_df.div(asys_summary_df["total"], axis=0) * 100
        ...
        for col in ['failed', 'complete']:
            if col not in asys_summary_df.columns:
                asys_summary_df[col] = 0

        # Plot
        asys_summary_df[["failed", "complete"]].plot(
            kind="barh", stacked=True, ax=ax, color=colors, width=0.7
        )

        # Optional: annotate failure % inside bars
        # for i, (fail_count, total) in enumerate(zip(asys_summary_df["failed"], asys_summary_df["total"])):
        #     if fail_count > 0:
        ...
        return table_path,acq_time_plot_path, acquisition_plot_path, data_comleteness_path, asysplot_path, asys_summary_df
    ...
    def qc_barplot(contrast, df):
        ...
        plt.subplots_adjust(hspace=0.3)  # Adjust spacing between the subplots
        plt.tight_layout()
        plot_path_artifacts = os.path.join(work_dir, f"artifact_failure_bar_{contrast}.png")
    ...
    for contrast in contrasts:
        try:
            ...
            if len(data.columns.tolist()) > 2:
                image1_path = qc_barplot(contrast,data)
                image3_path = plot_failures_over_time(contrast,df)
